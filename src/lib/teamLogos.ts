// NBA team logo URLs using ESPN CDN
...
export function getNBATeamLogo(teamCode: string): string {
  const normalized = teamCode.toUpperCase().trim();
  return NBA_TEAM_LOGOS[normalized] || '/placeholder.svg';
}
